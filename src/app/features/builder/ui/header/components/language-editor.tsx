...
import { compileLanguageObject } from "../../language";
...
export function LanguageEditor({
  ...
  // Get compiled language object from selected component
  ...
  // Get available languages from the language object
  ...
  // Update current language when language object changes
  useEffect(() => {
    if (languageObject) {
      try {
  ...
  const handleLanguageChange = (language: string): void => {
    if (languageObject) {
      try {
        languageObject.setLanguage(language, false); // Don't update URL
  ...
  // Don't render if no component is selected or no language support
  if (
    !selectedComponent ||
    !languageObject ||
